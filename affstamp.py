...
# Link analysis lives in pdflinkcheck, which also ships as a standalone tool.
# It reads the action out of the raw annotation object: PyMuPDF's
# page.get_links() reports a /Launch action as kind 5 - the same value it
# reports for a real /GoToR - so anything built on get_links() cannot tell
# "opens in the PDF viewer" from "opens in whatever owns .pdf on this
# machine", which is the distinction that matters most here.
...
def _fix_one_link(page, link, rel, open_in):
    """Rewrite a single link to `rel` under the chosen policy."""
    rect = pymupdf.Rect(link["raw"]["from"])
    if open_in == "browser":
        new = {"kind": pymupdf.LINK_URI, "from": rect, "uri": rel}
    elif open_in == "any":
        # "as-is" means exactly that: repair the path, change nothing else.
        if link["action"] == "/URI":
            new = {"kind": pymupdf.LINK_URI, "from": rect, "uri": rel}
        elif link["action"] == "/GoToR":
            new = {"kind": pymupdf.LINK_GOTOR, "from": rect, "file": rel,
                   "page": 0, "to": pymupdf.Point(0, 0)}
        else:
            new = {"kind": pymupdf.LINK_LAUNCH, "from": rect, "file": rel}
    else:                                       # viewer
        if rel.lower().endswith(".pdf"):
            new = {"kind": pymupdf.LINK_GOTOR, "from": rect, "file": rel,
                   "page": 0, "to": pymupdf.Point(0, 0)}
        else:
            # Not a PDF, so the viewer cannot display it in place.
            new = {"kind": pymupdf.LINK_LAUNCH, "from": rect, "file": rel}
    page.delete_link(link["raw"])
...
def _set_fit_destinations(doc):
    ...
    the page in PDF space, so the exhibit opens scrolled to the foot of its
    first page, and a zoom of 0 means nothing. /Fit shows the whole page.
    """
    for page in doc:
...
def cmd_links(a):
    ...
    warn_signed(doc, "base")
    open_in = getattr(a, "open_in", "viewer")

    def read_all(d):
        out = []
        for p in d:
            raw = {l.get("xref"): l for l in p.get_links()}
            for info in plc.read_links(d, p):
                info["page_no"] = p.number + 1
                info["raw"] = raw.get(info["xref"])
                info["page_obj"] = p
                out.append(info)
        return out
    ...
    rows = read_all(doc)
    print(BANNER)
    print("%s: %d pages, %d link annotation(s)   [should open in: %s]"
          % (os.path.basename(a.base), doc.page_count, len(rows), open_in))
    print(BANNER)

    absolute = [r for r in rows if uri_to_path(r["target"])]
    ...
    wrong_action = [r for r in rows
                    if r["target"] and not plc.WEB.match(r["target"])
                    and r["action"] not in plc.POLICY[open_in]["ok"]]
    if absolute:
        print("\n!! %d link(s) point at an absolute path. These break the "
              "moment the bundle moves." % len(absolute))
    if wrong_action and open_in != "any":
    ...
    if a.fix_relative:
        ...
        for r in rows:
            target = r["target"]
            if not target or plc.WEB.match(target) or r["raw"] is None:
                continue
            path = uri_to_path(target)
            needs_action = (open_in != "any"
                            and r["action"] not in plc.POLICY[open_in]["ok"])
            if not path and not needs_action:
                continue                        # already relative and correct
            ...
            if path:
                ...
                rel = target.replace("\\", "/")
            # A leading ./ is not part of the PDF file-specification grammar
            # and some macOS viewers reject it.
            while rel.startswith("./"):
                rel = rel[2:]

            _fix_one_link(r["page_obj"], r, rel, open_in)
            fixed += 1
            print("   p%d: %s\n        -> %s  (%s)"
                  % (r["page_no"], target, rel,
    ...
    if a.check:
        ...
        for r in rows:
            target = r["target"]
            if not target or plc.WEB.match(target):
                continue
            path = uri_to_path(target)
            if path:
                if not os.path.exists(os.path.normpath(path)):
                    missing.append((r["page_no"], target))
                continue
            status, detail = plc.resolve_target(target, root)
            if status == "missing":
                missing.append((r["page_no"], target))
            elif status == "case":
                miscased.append((r["page_no"], target, detail))

        print(BANNER)
        if missing:
            print("!! %d link target(s) do not exist under %s:"
                  % (len(missing), root))
            for pno, tgt in missing[:20]:
                print("     p%d: %s" % (pno, tgt))
        if miscased:
            print("!! %d link target(s) are spelled with different capitals "
                  "than the file on disk:" % len(miscased))
            for pno, tgt, real in miscased[:20]:
                print("     p%d: %s\n          on disk: %s" % (pno, tgt, real))
            print("   Windows and a stock Mac volume forgive this. A "
                  "case-sensitive volume, a network share, SharePoint or")
            print("   iManage will not, and the reader just gets "
                  "'cannot open the file'.")
        if not missing and not miscased:
            print("All file link targets resolve under " + root + ". Good.")
        return 1 if (missing or miscased) else 0
    return 0
...
def build_parser():
    ...
    p.add_argument("--open-in", choices=plc.OPEN_IN, default="viewer",
                   ...
                   help="how local links should open. viewer (default): "
                        "/GoToR, so the exhibit opens in the PDF viewer. "
                        "browser: /URI, handed to the default browser - more "
                        "reliable for recipients on macOS or a tablet. any: "
                        "leave the action types alone and only fix the paths.")
